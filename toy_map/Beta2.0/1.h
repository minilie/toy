...
public :
    ...
    BRTree(const BRTree &obj) : root(MakeNode::Cnode<TreeNode<T, U>, T, U>(this->pool, T(), U(), BLACK)), cmp(obj.cmp), node_cnt(0) {
        for (auto iter = obj.begin(); iter != obj.end(); iter++) {
            this->insertinter(MakeNode::Cpair<iterator, iterator>(this->pool, iter->first, iter->second));
        }
        return ;
    }
    ~BRTree() {
    ...
    TreeNode<T, U> &operator[](T key) {
        ...
        if (p == this->end()) {
            std::pair<T, U> k = MakeNode::Cpair<T, U>(this->pool, key, U());
...
RED_UP_MAINTAIN:
    ...
    std::pair<iterator, bool> insertinter(std::pair<T, U> p) {
        ...
        return MakeNode::Cpair<iterator, bool>(this->pool, insert_succeed, flag);
    }
