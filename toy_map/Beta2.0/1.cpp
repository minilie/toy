...
public :
    ...
    BRTree() : root(MakeNode<TreeNode<T, U>, T, U>::Cnode(this->pool, T(), U(), BLACK)), cmp(V()), node_cnt(0) {}
    BRTree (V cmp) : root(MakeNode<TreeNode<T, U>, T, U>::Cnode(this->pool, T(), U(), BLACK)), cmp(cmp), node_cnt(0) {}
    BRTree(const BRTree &obj) : root(MakeNode<TreeNode<T, U>, T, U>::Cnode(this->pool, T(), U(), BLACK)), cmp(obj.cmp), node_cnt(0) {
...
RED_UP_MAINTAIN:
    ...
    ptr insertimple(ptr root, T key, U val, iterator &succeed, bool &flag) {
        if (root == NIL) {
            ptr p = MakeNode<TreeNode<T, U>, T, U>::Cnode(this->pool, key, val);
            this->node_cnt += 1;
            succeed = p;
            flag = true;
            return p;
